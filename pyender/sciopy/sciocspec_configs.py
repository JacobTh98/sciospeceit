...
# EIT device Type : EitFe2020
# temperature measurement is not activated in autoMode_SineFit for the connected device
serial.write(bytearray([0xB0, 0x01, 0x01, 0xB0]))  # Reset measurement setup
serial.write(bytearray([0xC5, 0x00, 0xC5]))  # Not described
### External Module Port : 0x0 #callback
serial.write(bytearray([0xB0, 0x01, 0x01, 0xB0]))  # Reset measurement setup again
serial.write(bytearray([0xB0, 0x03, 0x02, 0x00, 0x09, 0xB0]))  #  Burst count
serial.write(
    bytearray([0xB0, 0x09, 0x05, 0x3F, 0x50, 0x62, 0x4D, 0xD2, 0xF1, 0xA9, 0xFC, 0xB0])
)
...
serial.write(bytearray([0xB1, 0x01, 0x03, 0xB1]))  # Get measurement setup
serial.write(bytearray([0xB2, 0x02, 0x01, 0x01, 0xB2]))  # Set output configuration
serial.write(bytearray([0xB2, 0x02, 0x03, 0x01, 0xB2]))
serial.write(bytearray([0xB2, 0x02, 0x02, 0x01, 0xB2]))
serial.write(bytearray([0xB4, 0x01, 0x01, 0xB4]))  # Start measurement
serial.write(bytearray([0xB4, 0x01, 0x00, 0xB4]))  # Stop measurement
